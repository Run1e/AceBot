...
class Tags(AceMixin, commands.Cog):
    ...
    async def make(self, ctx):
        ...
        while True:
            ...
            if name is None:
                ...
                await ctx.send(
                    ...
                    + "You can abort the tag creation by sending `{}abort` at any time.".format(
                        ctx.prefix
                    )
                )
        ...
        await ctx.send(
            "Tag `{0}` created! Bring up the tag contents by doing `{1}tag {0}`".format(
                name, ctx.prefix
            )
        )
    ...
    async def transfer(
        ...
        *,
        ...
        res = await self.db.execute(
            "UPDATE tag SET user_id=$1 WHERE id=$2", new_owner.id, record.get("id")
        )
