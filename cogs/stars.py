...
SB_LOCKED_ERROR = commands.CommandError(
    "Starboard has been locked and can not be used at the moment."
)
...
class Starboard(AceMixin, commands.Cog):
    ...
    async def create(self, ctx):
        ...
        if board.channel is not None:
            raise commands.CommandError(
                "Starboard already exists: {}".format(board.channel.mention)
            )
        ...
        if board.channel_id is not None:
            ...
            result = await ctx.prompt(
                title="The previous starboard seems to have been deleted.",
                prompt=prompt,
            )
        ...
        reason = "{} (ID: {}) has created a starboard channel.".format(
            ctx.author.display_name, ctx.author.id
        )

        topic = 'Star a message by adding a :star: reaction to it! Do "help star" to see all starboard related commands.'
        ...
        try:
            ...
            raise commands.CommandError(
                "An unexpected error happened when creating the starboard channel."
            )
    ...
    async def fix(self, ctx, *, message: StarConverter()):
        ...
        star_count = await self.db.fetchval(
    ...
    async def _on_star(self, board, starrer, star_channel, message, star_message, record):
        if record is None:
            ...
            if star_channel is None:
                raise commands.CommandError(
                    "I don't know where to post this. Try setting the starboard again."
                )
            ...
            if message.channel.is_nsfw() and not star_channel.is_nsfw():
                raise commands.CommandError(
                    "Can't star message from nsfw channel into non-nsfw starboard."
                )

            if not len(message.content) and not len(message.attachments):
                raise commands.CommandError(
                    "Can't star this message because it has no embeddable content."
                )
            ...
            starrer_count = await self.db.fetchval(
                "SELECT COUNT(*) FROM starrers WHERE star_id=$1", record.get("id")
            )
