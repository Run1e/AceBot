...
class Fun(AceMixin, commands.Cog):
        ...
        async with ctx.channel.typing():
            ...
            if not success:
                ...
                if means is not None:
                    val = (
                        ", ".join(x["val"] for x in means)
                        if isinstance(means, list)
                        else means["val"]
                    )
                    e.add_field(
                        name="Wolfram is having issues with these word(s):",
                        value="```{0}```".format(val),
                        inline=False,
                    )
    ...
    async def weather(self, ctx, *, location: str):
        ...
        params = {"access_key": APIXU_KEY, "query": location}
        ...
        async with ctx.channel.typing():
            ...
            e = disnake.Embed(
                title="Weather for {}, {} {}".format(
                    location["name"], location["region"], location["country"].upper()
                ),
    ...
    async def search(self, ctx, *, search: str):
        ...
        async with ctx.http.get(search_url) as resp:
            ...
            relevance = "**Relevancy:** {}%\n{}\n\n".format(
                str(round(float(results[0]) * 100, 2)), more_results
            )
