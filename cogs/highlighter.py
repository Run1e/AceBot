...
class Highlighter:
	...
	async def hl(self, ctx, *, code):
		...
		msg = await ctx.send('```{}\n{}\n```*{}, click the cross to delete.*'.format(self.guilds[ctx.guild.id] if ctx.guild.id in self.guilds else self.default, code, ('Paste by {}' if ctx.invoked_with else "Paste from {}'s link").format(ctx.message.author.mention)))
