...
class BaseParser:
    ...
    def __init__(self, page, prefix=None, postfix=None, ignores=None):
        ...
        self.h2m = DocsHTML2Markdown(
            ...
            base_url=self.DOCS_URL + self.page,
            big_box=False,
            lang="autoit",
            max_len=2000,
        )
    ...
    def _string_as_names(self, name):
        ...
        for name in names:
            if (
                not name.startswith("[")
                and len(name) > 5
                and re.match(MULTIPLE_NAME_RE, name)
            ):
                ...
                new_names.append(
                    (pre[:-1] if pre.endswith(" ") and post.startswith(" ") else pre)
                    + post
                )
    ...
    def get_desc_and_syntax(self, tag):
        ...
        def tag_process(current_tag):
            ...
            if (
                desc is None
                and current_tag.name == "p"
                and current_tag.get("class") is None
            ):
